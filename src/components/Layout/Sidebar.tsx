...
const navItems = [
  ...
  },
  {
    section: 'CRM',
    items: [
      { path: '/inquiries', label: 'Leads', icon: Users },
      { path: '/students', label: 'Students', icon: GraduationCap },
      { path: '/reminders', label: 'Follow-ups', icon: Bell },
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/products', label: 'Programs', icon: Package },
  ...
  const filteredSections = navItems
    .map((section) => ({
      ...
      items: section.items.filter((item) => {
        if (item.managerOnly && !['admin', 'counselor'].includes(user?.role || '')) return false;
        return true;
      }),
    }))
            ...
            <div className="space-y-5">
              {filteredSections.map((section) => (
                <div key={section.section}>
                  <p className="px-4 pb-2 text-xs font-semibold uppercase tracking-wide text-navy-400">
                    {section.section}
                  </p>
